...
<div class="w-full p-8 bg-white/80 rounded-3xl shadow-lg max-w-4xl mx-auto">
    ...
    <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        @csrf
        @method('PUT')
 <div id="deleted-images-container"></div>
        ...
        <div>
            ...
            <select name="penulis_id" required
                ...
                @foreach($penulis as $item)
                    <option value="{{ $item->id }}" {{ old('penulis_id', $post->penulis_id) == $item->id ? 'selected' : '' }}>
                        {{ $item->nama }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="status"
                class="w-full px-4 py-3 border border-green-200 rounded-2xl focus:outline-none">
                <option value="draft" {{ old('status', $post->status) == 'draft' ? 'selected' : '' }}>Nonactive</option>
       ...
       class="w-full px-4 py-3 border border-green-200 rounded-2xl focus:outline-none">
        ...
        <div class="md:col-span-2">
            ...
            <div id="thumbnail-preview" class="mt-2">
                @if($post->thumbnail)
                    <img src="{{ asset('storage/' . $post->thumbnail) }}" class="h-32 object-cover rounded-md shadow">
                @endif
            </div>
        ...
        <div class="md:col-span-2">
            ...
            <div id="image-inputs" class="space-y-4">
                ...
                <div class="flex items-center space-x-2 existing-image" data-id="{{ $img->id }}">
                    <img src="{{ asset('storage/' . $img->image) }}" class="w-32 h-32 object-cover rounded-md border">
                    <button type="button" class="remove-existing px-3 py-2 bg-red-500 text-white rounded-2xl text-xs hover:bg-red-600 focus:outline-none" data-id="{{ $img->id }}">
                        Hapus
                    </button>
                </div>
                @endforeach
        ...
        <div class="md:col-span-2">
            ...
            <textarea name="content" id="content" rows="6"
                class="w-full px-4 py-3 border border-green-200 rounded-2xl focus:outline-none">{{ old('content', $post->content) }}</textarea>
...
document.querySelectorAll('.remove-existing').forEach(button => {
    button.addEventListener('click', function() {
        let imgDiv = this.closest('.existing-image');
        let imageId = this.dataset.id;

        // tandai untuk dihapus di form
        let deletedContainer = document.getElementById('deleted-images-container');
        ...
        imgDiv.remove(); // hilangkan dari tampilan
    });
});
