...
@section('content')
    <div class="max-w-6xl mx-auto p-6 bg-white/80 rounded-3xl shadow-lg">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-max border border-gray-200 rounded-xl">
                    ...
                    @forelse($posts as $post)
                        <tr class="border-t hover:bg-gray-50">
                            ...
                            <td class="p-3">
                                ...
                                @else
                                    <div
                                        class="w-12 h-12 bg-gradient-to-br from-green-600 to-green-400 rounded-lg flex items-center justify-center text-white font-bold text-xs">
                            ...
                            <td class="p-3 text-center space-x-1 flex flex-nowrap justify-center gap-1 whitespace-nowrap">
                                <!-- Tombol Show -->
                                <a href="{{ route('posts.show', $post->id) }}"
                                    class="px-2 py-1 bg-gray-500 hover:bg-gray-600 text-white rounded text-xs md:text-sm flex items-center justify-center whitespace-nowrap">
                                    <i class="fas fa-eye mr-1"></i> Lihat
                                </a>
                                <!-- Tombol Edit -->
                                <a href="{{ route('posts.edit', $post->id) }}"
                                    class="px-2 py-1 bg-blue-500 hover:bg-blue-600 text-white rounded text-xs md:text-sm flex items-center justify-center whitespace-nowrap">
                                ...
                                <form action="{{ route('posts.destroy', $post->id) }}" method="POST" class="inline">
                                    ...
                                    <button onclick="return confirm('Yakin hapus post ini?')"
                                        class="px-2 py-1 bg-red-500 hover:bg-red-600 text-white rounded text-xs md:text-sm flex items-center justify-center whitespace-nowrap">
            ...
            <div class="flex flex-wrap space-x-1">
                ...
                @else
                    <a href="{{ $posts->previousPageUrl() }}"
                        class="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300 text-sm">«</a>
                @endif
                ...
                @foreach ($posts->getUrlRange(1, $posts->lastPage()) as $page => $url)
                    ...
                    @else
                        <a href="{{ $url }}"
                            class="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300 text-sm">{{ $page }}</a>
                    @endif
                @endforeach
