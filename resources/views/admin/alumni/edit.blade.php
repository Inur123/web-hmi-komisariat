...
<div class="w-full p-8 bg-white/80 rounded-3xl shadow-lg">
    ...
    <form action="{{ route('alumni.update', $alumnus->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="nama" value="{{ old('nama', $alumnus->nama) }}"
                class="w-full px-4 py-3 border border-green-200 rounded-2x focus:outline-none">
        ...
        <div>
            ...
            <textarea name="alamat" rows="3"
                class="w-full px-4 py-3 border border-green-200 rounded-2xl focus:outline-none">{{ old('alamat', $alumnus->alamat) }}</textarea>
        </div>
        ...
        <div>
            ...
            @if($alumnus->foto)
                <img src="{{ asset('storage/' . $alumnus->foto) }}"
