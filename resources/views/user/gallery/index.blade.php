            ...
            <a href="{{ route('gallery.show', $gallery->id) }}"
               class="block bg-white rounded-2xl shadow-lg overflow-hidden border border-green-100
                ...
                <div class="aspect-video relative overflow-hidden">
                    <img src="{{ asset('storage/' . ($gallery->images->first()->image ?? $gallery->thumbnail)) }}"
                         alt="{{ $gallery->nama }}"
                ...
                <div class="p-4">
                    ...
                    <p class="text-gray-400 text-xs">{{ \Carbon\Carbon::parse($gallery->tanggal)->translatedFormat('d F Y') }}</p>
                </div>
            </a>
            @empty
